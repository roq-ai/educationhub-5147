...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getStudentAssignments() {
    const data = await prisma.student_assignment
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'student_assignment'));
  ...
  async function createStudentAssignment() {
    ...
    const data = await prisma.student_assignment.create({
      data: body,
    });
    return res.status(200).json(data);
  }
